...
void bsp_InitUart(void)
{
	...
	bsp_InitUartDma((uint32_t)g_tUart3.pRxBuf, g_tUart3.usRxBufSize);  /*初始化串口3 RX DMA传输*/
}
...
void comClearTxFifo(COM_PORT_E _ucPort)
{
	...
	memset(g_tUart3.pRxBuf,0xAA,g_tUart3.usRxBufSize);
	bsp_InitUartDma((uint32_t)g_tUart3.pRxBuf, g_tUart3.usRxBufSize);
}
...
static void UartVarInit(void)
{
	...
	memset(g_tUart3.pRxBuf,0x5a,g_tUart3.usRxBufSize);  /*初始化接收缓存*/
#endif
...
//	}
	...
	{
		if(_pUart->pRxBuf[(_pUart->usRxRead + i) % _pUart->usRxBufSize ] != 0x5a)
		{
			*_pByte = _pUart->pRxBuf[_pUart->usRxRead];		/* 从串口接收FIFO取1个数据 */

			_pUart->pRxBuf[_pUart->usRxRead] = 0x5a;
